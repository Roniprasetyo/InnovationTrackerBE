    ...
    public class RencanaAnggaranKegiatanController(IConfiguration configuration, IWebHostEnvironment hostingEnvironment) : Controller
    {
        ...
        public async Task<IActionResult> SentRencanaAnggaranKegiatan([FromBody] dynamic data)
        {
            try
            {
                ...
                if (dt.Rows[0][0].ToString()!.Equals("SUKSES"))
                {
                    ...
                    htmlBody += "Nomor Registrasi RAK : <b>" + dt.Rows[0][1].ToString() + "</b><br><br>";
                    ...
                    for (int i = 0; i < dataProduk.Rows.Count; i++)
                    {
                        ...
                        htmlBody += "<td style='text-align:right;'>" + Utilities.Separator(Convert.ToInt64(Convert.ToDecimal(dataProduk.Rows[i][7].ToString())).ToString()) + "</td>";
                        htmlBody += "</tr>";
                        totalBiaya += Convert.ToDecimal(dataProduk.Rows[i][7].ToString());
                    }
                    htmlBody += "</table><br>";
                    ...
                    htmlBody += "Alternatif Keuntungan dan Diskon :<br><br>";
                    htmlBody += "<table border=1><tr><th>Alternatif #</th><th>Keuntungan (%)</th><th>Diskon (%)</th><th>Total Harga Jual (Rp)</th></tr>";
                    for (int i = 8; i <= 10; i++)
                    {
                        keuntunganAnggaran = totalAnggaran * Convert.ToDecimal(dataBiaya.Rows[0][i].ToString()) / 100;
                        htmlBody += "<tr>";
                        htmlBody += "<td style='text-align:center;'>" + (i - 7).ToString() + "</td>";
        ...
        public async Task<IActionResult> RejectRencanaAnggaranKegiatan([FromBody] dynamic data)
        {
            try
            {
                ...
                htmlBody += "Nomor Registrasi RAK : <b>" + dt.Rows[0][1].ToString() + "</b><br>";
