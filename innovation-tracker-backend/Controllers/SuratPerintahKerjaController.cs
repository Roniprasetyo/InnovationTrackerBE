        ...
        public async Task<IActionResult> SentSuratPerintahKerja([FromBody] dynamic data)
        {
            try
            {
                ...
                if (dt.Rows[0][0].ToString()!.Equals("SUKSES"))
                {
                    ...
                    htmlBody += "Sistem informasi produksi mencatat bahwa tim Marketing telah menerbitkan nomor Surat Perintah Kerja (SPK) baru dengan rincian sebagai berikut:<br><br>";
                    htmlBody += "Nomor Registrasi SPK : <b>" + dt.Rows[0][1].ToString() + "</b><br><br>";

                    footer += "Silakan lakukan pengecekan pada SPK tersebut dengan cara masuk terlebih dahulu ke <a href='" + configuration["Key:linkRoot"]! + "'>Sistem Informasi Produksi</a>.<br><br>";
                    ...
                    dtNotifikasi = lib.CallProcedure("all_createNotifikasiNew", [
                        ...
                        "<a href='surat_perintah_kerja'>Surat Perintah Kerja Baru No. " + dt.Rows[0][1].ToString() + " - Terbit</a>",
                        "[SISFO Produksi] - " + ldap.GetDisplayName(EncodeData.HtmlEncodeObject(value)[1]).ToUpper() + " Menerbitkan Surat Perintah Kerja No. " + dt.Rows[0][1].ToString(),
                        htmlBody,
                        footer,
                        "Detail",
